...
<template>
    <div class="container mt-16 text-main-text">
      ...
      <div v-else class="flex flex-col md:flex-row md:items-start items-center sm:justify-between">
        <div class="w-full sm:w-3/4" data-aos="zoom-out-right">
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="(item, index) in paginatedHealthKioskData" :key="index" class="relative mb-4" @click="goToHealthKiosk(item)">
              <img :src="item.img" :alt="item.name" class="w-full h-auto rounded-md">
              <p class="text-base sm:text-lg mt-2">{{ item.name }}</p>
              <div v-if="isExpired(item.finish_date)" class="absolute top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center text-white text-xl font-bold">
                VAXTI BİTİB
              </div>
            </div>
          </div>
...
const filteredHealthKioskData = computed(() => {
  return healthKioskData.value.filter(item => {
